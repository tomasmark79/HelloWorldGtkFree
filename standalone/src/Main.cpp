...
namespace Config {
  ...
  constexpr char standaloneName[] = "HelloWorldGtk";
...
std::unique_ptr<dotname::GtkLib> uniqueLib;
...
int processArguments (int argc, char* argv[]) {
  try {
    ...
    if (!result.count ("omit")) {
      // uniqueLib = std::make_unique<dotname::GtkLib> ();
      uniqueLib = std::make_unique<dotname::GtkLib> (Config::assetsPath, argc, argv);
    } else {
